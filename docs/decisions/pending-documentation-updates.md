...
| D3 | `docs/architecture/07-internal-control-plane.md` §20.2 | ExecutionResult contém sete campos: `commandId, outcome, observedRuntimeVersion, runtimeResourceIds, safeMetadata, errorCode, observeBeforeRetry` (Spec version 20.2) | ADR-0009 adiciona um oitavo campo: `observed`, um objeto `RuntimeObservation` contendo estado observado do recurso em runtime após um inspect bem-sucedido, normalizado apenas dentro do executor e consumido por diff/ownership/reconciliadores | Documento não editado. A divergência é registrada aqui e será corrigida por uma Story junto ao restante das atualizações de doc 07 (próximos Milestones), que integram adições graduais ao ExecutionResult conforme novos tipos de recurso (M01-18: services, M01-19: configs/secrets, M02-EXEC-SPLIT: observações de nó). | `M01-17` (ADR-0009) |
| D4 | `docs/architecture/08-networking-domains-edge.md` §5, tabela "Padrão recomendado" (linhas 162-173) | Exemplos de nomenclatura técnica: `net_prj01_env_prod` (14 caracteres) e `svc_prj01_prod_api` (17 caracteres), com projeto e ambiente no nome | Os identificadores ADR-0002 ULIDs de 26 caracteres + prefixo + separadores resultam em nomes maiores: `net_<projectId>_<environmentId>` = 65 caracteres, `svc_<projectId>_<environmentId>_<serviceId>` = 96 caracteres. O Engine Docker recusa nomes acima de 63 caracteres. ADR-0009 §5 remove a dependência do nome técnico para lookup (endereçamento por label de ownership), permitindo redução: `net_<environmentId>` (34 chars), `svc_<serviceId>` (34 chars). | Documento não editado; a tabela é um "Padrão **recomendado**" e seus exemplos predatam ADR-0002, não sendo, portanto, um contrato. A implementação segue doc 09 §195 (nomes determinísticos derivados de IDs, nunca como identificador primário) e ADR-0009 §5 (lookup por label). Sem impacto em contrato externo; os aliases de rede (`http://api:3000`) que aplicações usam são entidades separadas (doc 08 §6). | `M01-17` |
